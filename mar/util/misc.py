...
def fix_shape_mismatch(old_sd, new_sd, keys_to_tolerate):
    for k in keys_to_tolerate:
        if k in old_sd and k in new_sd:
            if old_sd[k].shape != new_sd[k].shape:
                print("Skipping loading {}, copying shape {} -> {}".format(
                    k, old_sd[k].shape, new_sd[k].shape
                ))
                old_sd[k] = new_sd[k]
