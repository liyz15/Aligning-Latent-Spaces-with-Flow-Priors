...
class AutoencoderKL(nn.Module):
    def __init__(self, embed_dim, ch_mult, use_variational=True, ckpt_path=None, reshape_factor=1):
        super().__init__()
    ...
    def encode(self, x):
        ...
        if self.reshape_factor > 1:
            # (B, C, H, W) -> (B, C * rs^2, H // rs, W // rs)
            assert moments.shape[2] % self.reshape_factor == 0
            assert moments.shape[3] % self.reshape_factor == 0
            moments = rearrange(moments, 'b c (h p) (w q) -> b (c p q) h w',
                                p=self.reshape_factor, q=self.reshape_factor)
    ...
    def decode(self, z):
        # (B, C * rs^2, H // rs, W // rs) -> (B, C, H, W)
        if self.reshape_factor > 1:
            z = rearrange(z, 'b (c p q) h w -> b c (h p) (w q)',
                          p=self.reshape_factor, q=self.reshape_factor)
