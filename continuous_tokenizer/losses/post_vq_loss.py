...
class PostVQLoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
        ...
        if self.discriminator_iter_start != 0:
            raise ValueError("Discriminator start iteration must be 0 when using generated latent loss.")
        ...
        if perceptual_loss == "vgg":
            self.perceptual_loss = LPIPS(grad_ckpt=grad_ckpt).eval()
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, 
                logger=None, log_every=100, info=None, reconstructions_gen=None):
        # generator update
        if optimizer_idx == 0:
            # reconstruction loss
            ...
            if reconstructions_gen is not None:
                reconstructions = torch.cat([reconstructions, reconstructions_gen], dim=0)
            ...
            if not isinstance(info, tuple):  # info is the latents for FlowAE, KLModel
                latent_norm = torch.linalg.norm(info, dim=-1).mean()
                sampled_latent = info[0, 0]
            else:
                latent_norm = 0.0
                sampled_latent = None
            ...
            if global_step % log_every == 0:
                ...
                flow_loss = codebook_loss[5] if len(codebook_loss) > 5 else 0.0
                logger.info(
                    f"(Generator) "
